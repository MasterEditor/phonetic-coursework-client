...
namespace Coursework_Client.ViewModels
{
    class RegisterViewModel : INotifyPropertyChanged
    {
            ...
            get
            {
                return sendSMSCommand ??
                    (sendSMSCommand = new RelayCommand(async obj =>
                    {
                        ...
                        RegisterRequestModel reg_model = new RegisterRequestModel();

                        reg_model.Number = Number.Replace("+", "");
                        reg_model.Password = Password;

                        var result = await apiClient.SendPostAsync(ApiStrings.Register, reg_model);
                    ...
                    {
                        if (string.IsNullOrEmpty(Number) || string.IsNullOrEmpty(Password) || string.IsNullOrEmpty(RepeatPassword)
            ...
            get
            {
                return registerCommand ??
                    (registerCommand = new RelayCommand(async obj =>
                    {
                        ...
                        ConfirmRegistrationRequestModel reg_model = new ConfirmRegistrationRequestModel();

                        reg_model.Id = SmsId;
                        reg_model.Code = ConfirmationCode;

                        var result = await apiClient.SendPostAsync(ApiStrings.ConfirmRegistration, reg_model);
        ...
        public bool Validate()
        {
            ...
            if (Number.Length < 6) { ShowError("Номер слишком короткий"); return false; }
            if (Password.Length< 6) { ShowError("Пароль слишком короткий"); return false; }
            if (RepeatPassword.Length< 6) { ShowError("Пароль слишком короткий"); return false; }
